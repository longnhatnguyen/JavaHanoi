...
namespace E.Contract.API.Controllers
{
    ...
    public class CategoryController : BaseController
    {
        ...
        // [Authorize(Roles = "CATEGORYCONFIGDETAIL")]
        [HttpGet("config-detail")]
        public async Task<IActionResult> Config()
        {
            try
            {
                var config = await this._categoryRepository.Config();
                return Ok(new ResponseSingleContentModel<ConfigModel>
                {
                    StatusCode = 200,
                    Message = "Lấy danh sách thành công.",
                    Data = config
        ...
        // [Authorize(Roles = "CATEGORYCONFIGUPDATE")]
        [HttpPost("config-update")]
        public async Task<IActionResult> ConfigModify([FromBody] ConfigModel model)
        {
            try
            {
                //model.userAdded = userid(_httpContextAccessor);
                var response = await this._categoryRepository.ConfigModify(model);
                return Ok(new ResponseSingleContentModel<ConfigModel>
                {
                    StatusCode = 200,
                    Message = "Cập nhật thành công",
